  ...
  <div class="w-[1/4] relative">
    <Sidebar>
      <SidebarItem
        :icon="LayoutGrid"
        text="Grid view"
        :active="curActiveView === ActiveView.Grid"
        @click="() => handleItemClick(ActiveView.Grid)" />
      <SidebarItem
        :icon="Rows"
        text="List view"
        :active="curActiveView === ActiveView.List"
        @click="() => handleItemClick(ActiveView.List)" />
      <SidebarItem
        :icon="Gem"
        text="Credits"
        :active="curActiveView === ActiveView.Credits"
        @click="() => handleItemClick(ActiveView.Credits)" />
      <SidebarItem
        :icon="Bug"
        text="Report bug"
        :active="curActiveView === ActiveView.Bug"
        @click="
          navigateTo(
      ...
      <SidebarItem
        ...
        :active="curActiveView === ActiveView.Feature"
        @click="
          navigateTo(
...
const handleItemClick = (view: ActiveView) => {
  ...
  curActiveView.value = curActiveView.value === view ? ActiveView.None : view
}
</script>
